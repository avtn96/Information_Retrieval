...
Original file is located at
    https://colab.research.google.com/drive/18brTDMJAdPKtF3kCG-B--lVaqeo29aV8
"""
...
def fetch_and_process_text(url):
    try:
        ...
        # Adjust token filtering to preserve numerical and mixed alphanumeric terms
        processed_tokens = [stemmer.stem(word.lower()) for word in tokens if word.lower() not in stop_words and (word.isalnum() or any(char.isdigit() for char in word))]
        return processed_tokens
    except Exception as e:
        print(f"Error processing URL {url}: {e}")
        return []

def calculate_aggregated_tf_idf(urls, words):
    aggregated_tokens = []
    document_presence = {stemmer.stem(word.lower()): 0 for word in words}

    # Aggregate tokens from all URLs
    for url in urls:
        ...
        for word in words:
            ...
            if stemmed_word in token_set:
                document_presence[stemmed_word] += 1

    # Calculate total term frequencies
    total_tokens = Counter(aggregated_tokens)
    total_words = sum(total_tokens.values())
    aggregated_tf = {word: total_tokens[stemmer.stem(word.lower())] / total_words if total_words > 0 else 0 for word in words}
    ...
    for word in words:
        stemmed_word = stemmer.stem(word.lower())
        if document_presence[stemmed_word] > 0:
            idf = math.log((total_documents / document_presence[stemmed_word]) + 1)  # Smoothing to avoid division by zero
            aggregated_tf_idf[word] = aggregated_tf[word] * idf
        else:
            aggregated_tf_idf[word] = 0

    return aggregated_tf_idf
...
aggregated_tf_idf_values = calculate_aggregated_tf_idf(urls, words)

for word, value in aggregated_tf_idf_values.items():
    print(f"Aggregated TF-IDF for '{word}': {value}")
